...
<script>
  ...
  export default{
    ...
    beforeRouteEnter(to,from,next) {
      next(vm => {
        if((from.name=="operationLog"|| from.name=="billPayDetail" || from.name=="orderInvite")&&localStorage.operationLogBackRoute=="bill"){
          vm.inow=1;
          vm.goTab(1);
        }else {
          vm.inow=0;
    ...
    beforeRouteLeave (to, from, next) {
      console.log(from.path.indexOf("overview"));
      if(from.path.indexOf("overview")!=-1){
        localStorage.operationLogBackRoute="overview";
      }else {
        localStorage.operationLogBackRoute="bill";
      }
      next();
    }
  }
