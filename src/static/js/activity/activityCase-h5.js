$(function() {
    ...
    $(".btn-case").on("click", function() {
        window.location.href = window.Host.local + "activity/activityLayout-h5.html?activityId="+activityId;
    });
        ...
        success: function(res) {
            ...
            if (data.length > 0) {
                $.each(data, function(i, index) {
                    ...
                    var oLi = $('<li class="case-item"></li>');
                    var str = '<a href='+url+'>';
                    str += '<div class="index-720" style="background-image:url('+index.caseCover+')">';
...
/**
 * [getMycaseList 获取我的预案列表]
 * @param  {[type]} activityId [活动id]
 * @param  {[type]} layoutId   [户型id]
 * @return {[type]}            [description]
 */
